...
export function useDashboardEGP() {
  ...
  const currency = 'EGP'
  const otherCurrency = 'PI'
  
  const collectionNames = {
    products: 'products_egp',
    orders: 'orders_egp',
    confirmedPayments: 'confirmedPayments_egp'
  }
  ...
  const handleSyncProduct = async (product) => {
    ...
    try {
      ...
      const otherCollection = 'products_pi'
      const syncedProduct = {
        ...product,
  ...
  // EGP: Filter by status === 'pending' AND !adminConfirmed
  const pendingOrders = orders.filter(order => 
    order.status === 'pending' && !order.adminConfirmed
  )
